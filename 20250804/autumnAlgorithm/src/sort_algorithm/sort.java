...
public class sort {
    ...
    //进行合并数组
    public static int[] merge(int[] leftNum, int[] rightNum){
        int len = leftNum.length+rightNum.length;
        int[] newNum = new int[len];
        int k=0;
        int i=0;
        int j=0;
        while (i<leftNum.length&&j<rightNum.length){
    ...
    //9 3  2 4
    //堆排序
    public static void fun_6(int[] nums){
        int n=nums.length;
        //建堆-从最后的非叶子节点至第一个节点
        for (int i=n/2;i>=0;i--){
            fun_7(nums,i,n);
    ...
    //对于n个元素进行调整堆
    public static void fun_7(int[] nums, int k, int n){
        //这循环条件
        while (k<n/2){
            int maxIndex = k*2+1;
            int rightIndex = k*2+2;
            //找出左右最大的-此时maxIndex指向最大的
